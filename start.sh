...
if [ "$ARCH" = "x86_64" ]; then
    ...
    /usr/bin/time -v ./rust/slideshowsetuprust-linux-amd64 \
      --db-path=$DB_PATH \
      --image-dir=$IMAGE_DIR \
    ...
    /usr/bin/time -v ./rust/slideshowsetuprust-linux-arm64 \
      --db-path=$DB_PATH \
      --image-dir=$IMAGE_DIR \
      --image-base-dir=$IMAGE_BASE_DIR \
      --http-prefix=$HTTP_PREFIX 
    ...
    /usr/bin/time -v ./rust/slideshowsetuprust-linux-armv7 \
      --db-path=$DB_PATH \
      --image-dir=$IMAGE_DIR \
      --image-base-dir=$IMAGE_BASE_DIR \
      --http-prefix=$HTTP_PREFIX
